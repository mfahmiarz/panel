...
abstract class AbstractPluginService
{
    ...
    /**
     * Get downloadable versions for a specific plugin.
     * Returns [['id' => '...', 'name' => '...', 'download_url' => '...'], ...]
     */
    abstract public function versions(string $pluginId, string $mcVersion = '', string $loader = ''): array;
    /**
     * Resolve the direct download URL for a given plugin + version.
     */
    abstract public function downloadUrl(string $pluginId, string $versionId): string;
    /**
    ...
    protected function getRedirectUrl(string $url): ?string
    {
        try {
            ...
            logger()->error('Error following redirect for plugin download', ['url' => $url, 'error' => $e->getMessage()]);
        }
        return null;
    }
